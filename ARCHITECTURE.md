...
`nix/setup.sh` is a slim orchestrator (~195 lines <!-- arch:max-lines nix/setup.sh 220 -->, ~125 of executable code). All logic lives in phase libraries; the orchestrator only sequences them and emits `_ir_flush` calls between phase transitions for crash-resilient provenance.
