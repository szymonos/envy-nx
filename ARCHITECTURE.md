...
| Linux prep | `.assets/scripts/linux_setup.sh` | Linux                    | Root, bash 4+                  |
...
Created by the corporate proxy certificate interception flow. See `docs/proxy.md` for operational
details.
...
`NODE_EXTRA_CA_CERTS`, `REQUESTS_CA_BUNDLE`, `SSL_CERT_FILE`, `UV_SYSTEM_CERTS`). See `docs/proxy.md` for
operational details.
...
Cross-phase globals follow a prefix convention that makes ownership and scope visible at a glance:
...
| lowercase   | Local / cross-phase     | `sorted_scopes`          | Various               |

### Wrapper boundary

The `_io_*` wrappers exist so tests can stub side effects. They apply at the **phase boundary** - functions in
`nix/lib/phases/*.sh` call `_io_nix`, `_io_curl_probe`, etc. Configure scripts (`nix/configure/*.sh`) are themselves
called via `_io_run` from the phase layer, so their internal commands (`gh`, `git config`, `curl`) are already
wrapped at the call site. Adding `_io_gh` inside `gh.sh` would require tests to stub at two levels for no gain.
...
**Bash/Zsh** (`nx.sh` renders blocks, `profile_block.sh` manages insert/remove):
...
Block rendering lives in `nx.sh` (`_nx_render_env_block`, `_nx_render_nix_block`). The `profiles.sh`/`.zsh` scripts
handle provisioning (file copy, CA certs, zsh plugins) then delegate block management to `nx profile regenerate`.
This means `nx profile regenerate` works standalone after the repo is removed.
...
**PowerShell** (`_aliases_nix.ps1` - `_NxProfileRegenerate` + region helpers):
...
Profile region rendering lives in `_aliases_nix.ps1` (native PowerShell). The `profiles.ps1` script handles
