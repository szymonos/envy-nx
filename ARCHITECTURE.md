...
phase_nix_profile_*     flake lock to the resolved rev, nix profile add/upgrade,
...
`nix/setup.sh` is a slim orchestrator (~225 lines <!-- arch:max-lines nix/setup.sh 230 -->, ~130 of executable code; the `--sync-only` and `--skip-configure` modes gate phases here rather than in the phase libraries, so the run shape stays readable in one file). All logic lives in phase libraries; the orchestrator only sequences them and emits `_ir_flush` calls between phase transitions for crash-resilient provenance.
...
| `playwright` | Linux: playwright-driver.browsers (Chromium); macOS: empty                | `playwright.sh` | uv tool `playwright` pinned to the nix browsers' version; profile exports `PLAYWRIGHT_BROWSERS_PATH`. macOS: latest CLI + `playwright install chromium`                          |
| `terraform`  | tfswitch, tflint                                                          | `terraform.sh`  | terraform binary installed via tfswitch to `~/.local/bin`                                                                                                                        |
| `docker`     | Linux: empty; macOS: colima, docker-client, docker-compose, docker-buildx | `docker.sh`     | Linux: root install via `install_docker.sh`. macOS: colima provides the VM, docker CLI talks to it, configure script writes the `envy-nx:certs` block into colima.yaml (see §3g) |
...
`nx.sh` is the entry point, containing shared helpers (notably `_nx_find_lib`, which uses `BASH_SOURCE` in bash and falls back to `$HOME/.config/nix-env` in zsh) + family file sourcing + the main dispatcher. Verb implementations live in four sibling family files split by domain:
...
| `nixpkgs_rev`           | locked nixpkgs matches the repo's validated revision; reports a failed upgrade attempt                                                   | WARN: behind, no rev file synced, or last upgrade failed                         |
...
| `managed_block_drift`   | Both managed blocks (`env:managed`, `nix:managed`) in the invoking shell's rc match what `nx profile regenerate --dry-run` would emit    | FAIL: rc-file blocks differ from regenerated content; WARN: nx not callable      |
...
- `nix:managed` - nix-specific (PATH, nix aliases, completions, prompt init). Removed by `nix/uninstall.sh`.
- `env:managed` - generic env (local PATH, cert env vars, generic aliases/functions). Survives uninstall.
...
**Env vars exported by the `env:managed` block:**
...
**colima VM cert mirroring (macOS only).** Host-side `ca-custom.crt` doesn't reach containers running inside the colima Lima VM - the VM has its own Linux CA store at `/usr/local/share/ca-certificates/`, and `docker pull` from inside fails with `x509: certificate signed by unknown authority` until the corp cert is trusted there too. `nix/configure/docker.sh` handles this via the colima YAML config: it upserts an `envy-nx:certs` sentinel block into colima's template (`~/.colima/_templates/default.yaml`, the source applied to every new instance) AND every existing `~/.colima/<profile>/colima.yaml`. The block declares a read-only mount of `~/.config/certs` -> `/mnt/envy-certs` and a `mode: system` provision script that on every boot copies `ca-custom.crt` into the VM's CA dir, runs `update-ca-certificates`, and restarts the docker daemon. Idempotency: the provision script's first line `cmp -s` short-circuits when content is unchanged. The lima `yaml.v3` parser rejects duplicate top-level keys, so the configure script strips the default-empty `mounts: []` / `provision: null` scaffolding lines that colima writes when it first creates a profile - users with non-default values for either key get a warning and the profile is left alone. Block insertion/replacement uses the same `manage_block` helper as bash/zsh rc files (`profile_block.sh`), with the same atomic-rename + duplicate-block-collapse guarantees.

### 3h. WSL host orchestration (`wsl/wsl_setup.ps1` + `modules/utils-setup/`)

`wsl/wsl_setup.ps1` runs on the Windows host. It enumerates distros, installs missing ones, and per-distro: runs `check_distro.sh`, resolves scopes, runs DNS+SSL probes (with optional fixes), bootstraps nix via `wsl.exe --exec nix/setup.sh`, syncs SSH keys / GitHub config, configures git, and writes per-distro `install.json` provenance. The orchestrator is a slim ~350-line dispatcher <!-- arch:max-lines wsl/wsl_setup.ps1 400 -->; per-distro phase logic lives in 16 named functions on the existing `modules/utils-setup` PowerShell module:

| File                                               | Functions                                                                                                                                                                                                                                   |
| -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `modules/utils-setup/Functions/wsl_common.ps1`     | `Get-WslDistro`, `Set-WslConf`, `Invoke-WslExe` (generic WSL plumbing)                                                                                                                                                                      |
| `modules/utils-setup/Functions/wsl_install.ps1`    | `Install-WslDistroIfMissing`, `Install-WslService`, `Invoke-WslDistroMigration`, `Get-WslMigrationChoice`, `Get-WslFileFromDefault`, `Get-WslArchiveFromDefault`, `Resolve-WslGtkThemePreference` (begin block)                             |
| `modules/utils-setup/Functions/wsl_phases.ps1`     | `Resolve-WslDistroScopes`, `Invoke-WslDistroCheck`, `Invoke-WslBaseSetup`, `Sync-WslGitHubConfig`, `Sync-WslNetrc`, `Expand-WslArchive`, `Sync-WslSshKeys`, `Install-WslScopes`, `Set-WslGtkTheme`, `Set-WslGitConfig` (per-distro process) |
...
**Test coverage.** `tests/pester/WslSetup.Tests.ps1` exercises the orchestrator end-to-end with `wsl.exe` Mocked at both script scope and inside the `utils-setup` module scope. `tests/pester/WslSetupPhases.Tests.ps1` covers 14 of the 16 extracted functions directly (`Install-WslService` is exercised indirectly through `Install-WslDistroIfMissing`'s failure path; `Get-WslMigrationChoice` wraps `$Host.UI.PromptForChoice` and is interactive-only).

**WSL CI gap.** GitHub-hosted Windows runners only support WSL1 (no nested virtualization), which lacks systemd and behaves differently from production WSL2. End-to-end testing is intentionally omitted - the orchestration logic is validated by the Pester tests above.
...
**No repo-level `flake.lock`.** Per-user lock in `~/.config/nix-env/flake.lock` gives run-to-run reproducibility on one machine; explicit `nx pin` covers fleet pinning. A lock file would pin every input and fight `nx pin`'s `--override-input`, so the repo ships a single scalar instead - `nix/nixpkgs_rev.json`, the CI-validated revision (decision [0011](design/decisions/0011-validated-nixpkgs-rev.md)). `nixpkgs-unstable` is still the flake input, but users resolve to the validated revision rather than HEAD; `nx upgrade --latest` opts out. Every `setup.sh` run (and so `nx setup` / `nx upgrade`) moves to the resolved revision, restoring the previous `flake.lock` from the EXIT trap if the profile never receives it; `nx self update` runs `setup.sh --sync-only`, which stops after syncing `~/.config/nix-env/`, and `nx upgrade` runs `setup.sh --skip-configure`, which skips `phase_configure_*` and `phase_post_install_common` (decision [0012](design/decisions/0012-one-upgrade-path.md)).
...
3. Regenerate everything: `python3 -m tests.hooks.gen_nx_completions`. This rewrites the bash/zsh/PS completers, `_nx_lifecycle_help` body, `_nx_verb_help`, `nx_main` case body in `nx.sh`, and (if the verb is `nx profile *`) the PS `nx:dispatch` region in `_aliases_nix.ps1`. **No hand-editing of the dispatcher.**
...
Rare - only when an existing family becomes too large or a new domain emerges. The lib-file list is now derived from the manifest's `family` fields and **generated** into both sync/audit sites - no manual list-editing.
...
3. Set `"family": "<family>"` on at least one verb (or subverb-verb) in `nx_surface.json`. The generator picks up unique families and emits `nx_<family>.sh` to both sync/audit sites.
...
### 7.10. Never use bare `mv` / `cp` in interactively-sourced shell files

Files sourced into the user's interactive shell (the set in `tests/hooks/_file_scopes.py::INTERACTIVE_SHELL`: nx CLI family files, `nx_doctor.sh`, `profile_block.sh`, `helpers.sh`, `certs.sh`, `shell_cfg/*.sh`) inherit the user's aliases and shell functions. A common defense against accidental clobber is `alias mv='mv -i'` (or `mv -iv`) and `alias cp='cp -i'`, written into the user's `~/.bashrc` or `~/.zshrc`. When our sourced functions call bare `mv` or `cp`, the alias expansion fires and the command prompts on stdout - in any non-tty context (script, CI hook, IDE-spawned subshell) the prompt silently returns no input and the command exits non-zero, leaving the destination file unchanged. The caller sees the apparent success of the surrounding pipeline and the broken state only surfaces later.

Concrete incident: `_nx_write_pkgs` in `nx.sh` did `mv "$tmp" packages.nix`. Under `alias mv='mv -iv'`, `nx install <pkg>` hung at the `overwrite packages.nix?` prompt; subsequent `nix profile upgrade` ran against the unchanged file and silently kept the user's old package set.

**Rules:**

1. **Always prefix `mv` and `cp` with `command`** in interactively-sourced files. `command` bypasses both aliases and shell-function shadows; `\mv` works too but is less idiomatic. ShellCheck and the hook accept both.
2. **`rm -f` is safe** under `alias rm='rm -i'` because `-f` always wins over `-i` on both BSD and GNU `rm`. Bare `rm <file>` (no `-f`) is not safe; current call sites all use `-f`.
3. **Other builtins** (`cat`, `ls`, `grep`) are generally safe under common aliases because the aliases typically add flags (`alias ls='ls --color=auto'`) rather than triggering interactive prompts. If a future alias DOES introduce prompting behavior, extend the hook.

**Enforced by `check-no-aliased-builtins`** (`tests/hooks/check_no_aliased_builtins.py`): forbids unprefixed `mv`/`cp` in the `INTERACTIVE_SHELL` file set. The file list lives in `tests/hooks/_file_scopes.py` as a single source of truth (also referenced by `.pre-commit-config.yaml`'s `files:` regex via a comment). Suppress with `# alias-ok` on the same line if the call genuinely should honor user aliases (rare in libraries).

### 7.11. Every distro `case` needs a `*)` arm

An unmatched `/etc/os-release` ID leaves the distro variable as the **empty string**, not unset. A `case $SYS_ID in` with no `*)` arm therefore runs no arm at all: bash succeeds, `set -e` sees nothing to trip on, and the script exits 0. `install_gh.sh` printed "installing github-cli", installed nothing, skipped the `/etc/apt/keyrings` signing key its apt source names as `signed-by=`, and returned success. `vim_setup.sh global` was worse - it deleted the user's and root's vim config *before* a case with only three arms, so on alpine/arch/opensuse the configuration was destroyed and nothing replaced it. shellcheck has no opinion here: a `case` with no default is valid shell.

**Rules:**

1. **Every `case` on the distro variable ends in `*)`.** The arm must be last (arms after it are dead code) and must have a non-empty body.
2. **Guard both cases, not one.** Installers typically have a detection case and an install case. Guarding only one relies on execution order that the next edit can reverse - the `system-installers` review found two files guarded on the detection side and two on the install side, all four "correct" by accident.
3. **Reuse the house arm verbatim.** Copy it from `.assets/provision/install_podman.sh`: read the raw `ID=` field from `/etc/os-release` (`SYS_ID` is empty by construction in that arm, so it cannot name the distro), two red `printf`s to stderr, `exit 1`. Byte-identical across installers so the next copy-paste inherits a correct arm. `install_base.sh`'s soft-skip arm and `vim_setup.sh`'s narrower "Supported:" line are the documented deviations.
4. **Do not require all six distro arms.** `install_docker.sh` omits alpine deliberately, `fix_azcli_certs.sh` has four arms, `functions.sh` five.

**Enforced by `check-distro-case-arm`** (`tests/hooks/check_distro_case_arm.py`). Scope is **content-based, not directory-based** - a file is in scope when it assigns a variable from a distro-id alternation group, including narrowed variants like `(fedora|debian|ubuntu|opensuse)`. Directory scoping is exactly how the next copy-paste sibling would escape the rule. Only `case` statements whose subject is one of those variables are judged, so a subcommand dispatch (`case "${1:-}" in` in `fix_wsl_dns.sh`) is untouched. Suppress with `# distro-case-ok: <reason>` on the `case` line; the reason is mandatory and must name what handles the unmatched distro instead (`check_ssl.sh` degrades to a probe ladder its caller parses; `fix_azcli_certs.sh` has a venv fallback).

### 7.12. Every curl/wget fetch carries `--proto` and `--tlsv1.2`

Without `--proto '=https'` curl will follow a plaintext redirect hop and pipe the result into `sh`; without `--tlsv1.2` it will negotiate TLS 1.0 with whatever middlebox is in the way. These flags are house style precisely because nobody re-derives them per script - the `system-installers` review found them on some installers and absent on their copy-paste siblings, and found a `-k` in `download_file` that disabled verification for every caller.

`--proto-redir` is **not** required. curl(1): "Protocols denied by `--proto` are not overridden by this option" - `--proto` already constrains every redirect hop. Call sites that pass both are fine; do not drop `--proto` in favour of `--proto-redir`, which would leave the initial request unguarded.

**Rules:**

1. **`curl --proto '=https' --tlsv1.2 ...`** on every executing fetch. `--tlsv1.3` is accepted as a higher floor.
2. **Prefer `download_file`** (`.assets/lib/helpers.sh`) over a hand-rolled curl - it is compliant once, with retry and `%{http_code}` handling. Call sites that use it (`nix/configure/conda.sh`, `install_gh.sh`, `install_gcloud.sh`) need no flags of their own.
3. **wget has no `--proto`.** It needs `--secure-protocol=TLSv1_2` instead. Prefer curl for new code.
4. **Connectivity probes are the only exception.** A probe that discards the body, takes a user-overridable URL that may be `http://`, or relies on `-k` to distinguish "cert rejected" from "unreachable" would change meaning under the flags.

**Enforced by `check-curl-tls`** (`tests/hooks/check_curl_tls.py`) over `.assets/` and `nix/`. The hook only treats curl/wget as a fetch when the token stands in **command position**, which is what excludes package names in installer arms (`apk add ... curl`), existence probes (`command -v curl`, `for cmd in curl jq tar`, `has_system_cmd curl`) and `-v`-style arguments without any special-casing. On top of that it strips `#` comments and the `: '...'` runnable-examples block, skips strings passed to output helpers (`err "  curl ... "` remediation hints in `bootstrap.sh`) and alias definitions, treats a quoted string as executing only when an exec-ish `-c "..."` runs it (`install_nix.sh`'s `su - "$SUDO_USER" -c "curl ..."`), and resolves flag sets held in a variable (that same call's `$curl_flags`). Suppress with `# tls-probe-ok: <reason>`; reason mandatory. Out of scope for v1: `.assets/config/vim/.vimrc` (vimscript) and `.assets/docker/Dockerfile.*` (needs Dockerfile parsing).
...
| `check-no-aliased-builtins` | `check_no_aliased_builtins.py` | Forbids bare `mv` / `cp` in interactively-sourced shell files (file list owned by `tests/hooks/_file_scopes.py::INTERACTIVE_SHELL`). Users with `alias mv='mv -i'` would prompt-hang `nx install` / `nx scope` etc. - use `command mv` / `command cp`. Suppress with `# alias-ok`. See §7.10.             |
| `check-distro-case-arm`     | `check_distro_case_arm.py`     | Requires a non-empty, last-position `*)` arm on every `case` whose subject is a distro variable. **Content-scoped** (any file that detects a distro id), not directory-scoped. Suppress with `# distro-case-ok: <reason>`. See §7.11.                                                                     |
| `check-curl-tls`            | `check_curl_tls.py`            | Requires `--proto` and `--tlsv1.2` on every *executing* curl/wget fetch in `.assets/` and `nix/`. Skips comments, examples blocks, output-helper strings, package names, existence probes; resolves flags held in a variable. Suppress with `# tls-probe-ok: <reason>`. See §7.12.                        |
| `check-md-html-tags`        | `check_md_html_tags.py`        | Forbids unbalanced tag-like `<...` in `docs/*.md` - Python-Markdown's HTML preprocessor eats them and silently drops the rest of the page                                                                                                                                                                 |
...
| `check-learning-trailer`    | `check_learning_trailer.py`    | Commit-msg stage: nudges for a `Codified-Learning:` trailer when high-leverage paths are staged. Skippable with `# no-learning`                                                                                                                                                                           |
| `check-nx-generated`        | `check_nx_completions.py`      | All generated artifacts match `nx_surface.json`: bash/zsh/PS completers, `_nx_lifecycle_help` body, `_nx_verb_help`, `nx_main` case arms, PS `nx:dispatch` region, lib-file for-loops in 2 sync/audit sites. Regenerate via `python3 -m tests.hooks.gen_nx_completions`. Replaces 4 earlier parity hooks. |
| `check-arch-drift`          | `check_arch_drift.py`          | Agent-facing docs match the tree: every backtick-quoted repo path exists, and every `<!-- arch:max-lines -->` budget still holds. Covers this file, `CONTRIBUTING.md`, `.claude/rules/*.md`, `design/**`. A design proposal opts out of the path check with `<!-- arch-drift: proposal -->`.              |
...
- `nix/uninstall.sh --env-only` removes the `nix:managed` block, preserves the `env:managed` block, leaves `/nix/store` intact, and (when `--conda` was in scope) removes `~/miniforge3/` plus the `conda initialize` rc block.
...
**All-scopes env build** (`test_scope_env.yml`, local equivalent `make test-scope-env`). The integration matrices install a handful of scopes each, so a package that collides with a package from a *different* scope only breaks the users who combine them - the way minikube's `bin/kubectl` symlink took down every env that had both `k8s_ext` and `k8s_base` (v1.19.4). `tests/nix/build_scope_env.sh` generates a `config.nix` with every scope in `scopes.json`, builds the flake (a `pkgs.buildEnv` collision fails the build), then asserts every binary each scope declares in its `# bins:` comment exists in the result, and that pinned bins resolve to the expected package. It runs on PRs touching `nix/scopes/`, `nix/flake.nix`, `nixpkgs_rev.json` or `scopes.json`. It holds the *revision* fixed (whatever `nixpkgs_rev.json` pins) and varies the *repo*, so it answers "does this scope change break the env?"; `bump_nixpkgs_rev.yml` is the mirror image and never runs on a PR, so neither subsumes the other. It carries no schedule - a weekly rebuild of the same pinned revision is nearly a no-op, and drift detection is the bump gate's job. Known collisions are resolved in `flake.nix:collisionLowPrio`, which demotes the losing package with `lib.lowPrio` (it still installs, minus the clashing file) and covers ad-hoc `nx add` packages too.

**Validated nixpkgs revision** (`bump_nixpkgs_rev.yml`). The flake tracks `nixpkgs-unstable`, which Hydra validates against nixpkgs' own tests - not against this repo's scope combinations, which is why the v1.19.4 `bin/kubectl` collision passed upstream CI and still reached users. `nix/nixpkgs_rev.json` records the revision users install; the weekly workflow advances it only after a candidate has built every scope and completed a real `setup.sh` install on **both** Linux and macOS. The gate runs inline before the PR is opened - a PR created with `GITHUB_TOKEN` does not trigger `pull_request` workflows, and `main` has no required checks for `gh pr merge --auto` to wait on. Revision precedence (`_nx_rev_resolve` in `.assets/lib/nx_rev.sh`, shared by `nx upgrade` and the `nix_profile` phase): `nx pin set` > `nx upgrade --latest` > validated rev > keep the current lock. It never falls through to HEAD. Two repo variables make the file safe to inherit in downstream redistributions: `NIXPKGS_BUMP_ENABLED=false` inherits upstream's revision instead of picking its own (the default choice - a copy on the same runners diverges for identical signal), and `NIXPKGS_BUMP_AUTOMERGE=false` leaves the PR for review where a ruleset requires one. See decision [0011](design/decisions/0011-validated-nixpkgs-rev.md).
...
| `modules/utils-setup/Functions/wsl_phases.ps1`     | Per-distro process phases: `Invoke-WslDistroCheck`, `Invoke-WslBaseSetup`, `Sync-WslGitHubConfig`/`Sync-WslNetrc`/`Expand-WslArchive`/`Sync-WslSshKeys`, `Install-WslScopes`, ... |
