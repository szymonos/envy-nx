...
**Test coverage.** `tests/pester/WslSetup.Tests.ps1` (10 integration tests <!-- arch:count tests/pester/WslSetup.Tests.ps1 '^\s*It ' 10 -->) exercises the orchestrator end-to-end with `wsl.exe` Mocked at both script scope and inside the `utils-setup` module scope. `tests/pester/WslSetupPhases.Tests.ps1` (59 unit tests <!-- arch:count tests/pester/WslSetupPhases.Tests.ps1 '^\s*It ' 59 -->) covers 14 of the 16 extracted functions directly (`Install-WslService` is exercised indirectly through `Install-WslDistroIfMissing`'s failure path; `Get-WslMigrationChoice` wraps `$Host.UI.PromptForChoice` and is interactive-only).
...
| `check-arch-drift`          | `check_arch_drift.py`          | Agent-facing docs match the tree: every backtick-quoted repo path exists, and every `<!-- arch:max-lines -->` budget / `<!-- arch:count -->` claim still holds. Covers this file, `CONTRIBUTING.md`, `.claude/rules/*.md`, `design/**`. A design proposal opts out of the path check with `<!-- arch-drift: proposal -->`. |
