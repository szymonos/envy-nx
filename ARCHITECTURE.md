...
**Test coverage.** `tests/pester/WslSetup.Tests.ps1` (9 integration tests <!-- arch:count tests/pester/WslSetup.Tests.ps1 '^\s*It ' 9 -->) exercises the orchestrator end-to-end with `wsl.exe` Mocked at both script scope and inside the `utils-setup` module scope. `tests/pester/WslSetupPhases.Tests.ps1` (56 unit tests <!-- arch:count tests/pester/WslSetupPhases.Tests.ps1 '^\s*It ' 56 -->) covers 14 of the 16 extracted functions directly (`Install-WslService` is exercised indirectly through `Install-WslDistroIfMissing`'s failure path; `Get-WslMigrationChoice` wraps `$Host.UI.PromptForChoice` and is interactive-only).
