...
- Script-scoped variables: `$script:varName` for state shared across function boundaries
...
- Check `$LASTEXITCODE` immediately after external command invocations (`wsl.exe`, native binaries)
- Use `Show-LogContext` for structured logging with `-Level` (INFO, WARNING, ERROR, DEBUG) and `-ErrorStackTrace $_.ScriptStackTrace`
...
- Normalize line endings with `.Replace("`r`n", "`n")` for cross-platform (Windows/Linux) consistency
...
- Use `Invoke-WslExe` wrapper for native wsl commands (--install, --update) to preserve UTF-16 output
