...
def main(argv: list[str]) -> int:
    ...
    # only enforce during commits, not lint sweeps (--all-files)
    if not any(f in staged for f in runtime_files):
        return 0

    if has_unreleased_content(CHANGELOG) or "CHANGELOG.md" in staged:
        return 0
