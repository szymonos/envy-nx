...
# -- resolve_scope_deps idempotency -------------------------------------------

@test "resolve_scope_deps is idempotent" {
  scope_add "k8s_ext"
  resolve_scope_deps
  local first="$_scope_set"
  resolve_scope_deps
  [[ "$_scope_set" == "$first" ]]
}

@test "resolve_scope_deps with pre-added deps produces no duplicates" {
  scope_add "k8s_ext"
  scope_add "k8s_base"
  resolve_scope_deps
  local count
  count="$(echo "$_scope_set" | grep -o 'k8s_base' | wc -l)"
  [[ "$count" -eq 1 ]]
}
