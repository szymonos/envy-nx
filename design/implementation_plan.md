...
| Provenance       | `install.json` EXIT trap across all entry points, `nx version`          |
...
Docker, docs. Generates `CHECKSUMS.sha256`.
...
- `docs/ops/` - overlay authoring guide, air-gapped install, corporate
  proxy setup (move existing `docs/proxy.md` here).
