...
The `grep -q 'pattern' || echo 'line' >> ~/.bashrc` pattern is fragile: duplicates on re-run, no in-place update, no clean removal. This tool uses sentinel-delimited managed blocks (`# >>> nix:managed >>>` / `# <<< nix:managed <<<`) that are fully regenerated on each run.
