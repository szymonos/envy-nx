...
`make test-nix` builds a throwaway Docker image that runs a full nix provisioning pass, verifies the `nx` CLI, validates `install.json`, and tests the uninstaller - all in an isolated container. Slower than unit tests, but catches integration issues that mocking cannot.
