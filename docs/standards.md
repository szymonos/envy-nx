...
| Metric                       | Value                                                                       |
| ---------------------------- | --------------------------------------------------------------------------- |
| Unit test files              | 37 (28 bats + 9 Pester)                                                     |
| Individual test cases        | 676 (534 bats + 142 Pester)                                                 |
...
28 bats files cover bash logic: scope dependency resolution, `nx` CLI commands and tab completers, managed-block injection / removal, profile migration, the overlay system, health checks, certificate handling, manager-scope removal hooks (`conda`, `nodejs`, `python`), the colima.yaml cert provisioner, the macOS VS Code settings writer, and zsh runtime smoke tests.
