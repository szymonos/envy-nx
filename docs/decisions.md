...
This pattern makes bash scripts testable at a level normally associated with compiled languages - without mocking frameworks, without PATH manipulation, without subprocess overhead. It is the reason this project has 665 test cases across 36 test files for what is, at its core, a shell script.
...
**This is review machinery, not enforcement.** The framework produces findings; the human decides which to act on. There is no auto-merge, no policy gate, no "the reviewer said critical so we must fix." The reviewer is a colleague with fresh eyes, not an authority. The whole framework lives under [`design/reviews/`](https://github.com/szymonos/envy-nx/tree/main/design/reviews) and the orchestrating `/review` skill - the supporting agents, charters, and accepted-decisions ledger version with the repo so the next maintainer inherits the same vocabulary.
