...
- `check-nx-dispatch-parity` pre-commit hook (`tests/hooks/check_nx_dispatch_parity.py`): asserts every verb (and alias) in `nx_main`'s `case "$cmd" in` block matches `nx_surface.json` and vice versa. Closes the third drift loop opened by the manifest system - completers (`check-nx-completions`), PS profile dispatcher (`check-nx-profile-parity`), and now the bash dispatcher itself. Caught the `add`/`ls` drift on first run.
- `nx help` text is now generated from `nx_surface.json` by `tests/hooks/gen_nx_completions.py`. The `_nx_lifecycle_help` function body is wrapped with `# >>> nx-help generated >>>` / `# <<< nx-help generated <<<` markers and replaced in-place. Verb summaries, args representations (`<query>`, `<packages...>`, `[flags...]`), and the `(nx <verb> help)` hint for verbs with subverbs all derive from the manifest. `check-nx-completions` extended to verify the help region matches what the generator would emit. Adds an optional `help_args` field to the schema (used by `setup` to render `[flags...]` since its primary surface is passthrough flags, not positional args). Single source of truth for verb summaries: previously the same descriptions lived in both the heredoc and the manifest's `summary` field.
...
- `nx` tab completers (`completions.bash`, `completions.zsh`, the `nx-completer` region in `_aliases_nix.ps1`) are now generated from `nx_surface.json` by `tests/hooks/gen_nx_completions.py`. Adding a verb or flag is a one-file edit (the manifest) instead of a four-file edit. Side effects: zsh completer now offers `nx doctor` flags (`--strict`, `--json`) and `nx overlay` subverbs (`list`, `status`) which were previously missing; bash and zsh both now complete subverb aliases (e.g. `rm` for `scope remove`); bash groups subverbs sharing a completer into one elif (4 lines instead of 12 for `scope show|edit|remove|rm`).
- `check-nx-profile-parity` pre-commit hook (`tests/hooks/check_nx_profile_parity.py`): asserts the bash and PowerShell `nx profile` dispatchers expose the same subverbs. The two dispatchers are intentionally independent (they manage structurally different files - bash/zsh rc vs PowerShell `$PROFILE`) but their user-facing surface must stay in sync. Caught real drift on first run: PS dispatcher had no explicit `'help'` case (fell through to `default`); fixed by extracting the help text into `_NxProfileHelp` and dispatching both `'help'` and `default` to it, matching bash's `help | *)` pattern. ARCHITECTURE.md "Managed block pattern" section refreshed to clarify the symmetry is by necessity, not a duplication smell.
- `check-nx-completions` pre-commit hook (`tests/hooks/check_nx_completions.py`): asserts the committed completer files match what the generator would emit from `nx_surface.json`. Catches drift in either direction - someone hand-editing a completer or editing the manifest without regenerating. Triggers on changes to the manifest, any of the three generated files, or the generator/checker itself.
